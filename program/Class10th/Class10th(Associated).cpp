...
#pragma region 레드블랙트리
	
	//레드블랙트리의 핵심 속성과 불일치할때,
	// 삼촌 노드가 검은색이라면 ->Restructuring
	// 문제가 되는 노드들의 조상부터 자식까지의 값을 오름차순으로 배열,
	// 이진트리 공식에 따라 재배치+기존 색 유지
	//
	// 
	// 
	//Recoloring
	//1.루트 노드는 무조건 블랙이여야함,
	// 2.다만 조상 노드을 검은색으로 변경해야함,
	// 1에 의해서 루트가 검은색이기 때문에 그 아래 노드색을 변경
	//더블래드는 안되지만, 더블 블랙은 문제 없음
...
//제곱탐사와 이중 해시는 충돌을 해결하기 위한 방법중 개방주소법3가지중 2가지입니다.
// 
//제곱탐사는 충돌이 발생했을때 해시값에 제곱을 더해서 다음 버킷을 찾는 방법입니다.

//장점: 해시 충돌이 발생했을 때, 다음 버킷을 찾는 과정에서 충돌이 덜 발생할 수 있습니다.
//1차 군집화 문제 완화 : 선형 탐사처럼 충돌이 발생한 항목들이 연속적으로 뭉쳐 다니는 현상(군집화)을 완화합니다. 
//초기 해시 값이 같더라도 탐사 경로가 더 넓게 퍼지므로, 빈 버킷을 찾을 확률이 높아집니다.

//단점:2차 군집화(Secondary Clustering) 문제 : 동일한 최초 해시 값을 가진 키들은 항상 동일한 탐사 시퀀스를 따릅니다.
//같은 시작점에서 충돌한 키들은 같은 경로를 따라 움직이므로, 여전히 군집화가 발생할 수 있습니다.
//테이블의 모든 버킷을 탐사하지 못할 수 있습니다.

//이중 해싱
//개념: 해시 충돌이 발생했을 때, 두 번째 해시 함수를 사용하여 탐사 간격을 결정하는 방법입니다.  
//탐사 공식 : H(key, i) = (H_1(key) + i⋅H_2(key))(modM) 여기서 :

//	H(key, i) : i번째 탐사 시의 최종 버킷 인덱스
//	H_1(key) : 최초 해시 함수로 계산한 인덱스
//	H_2(key) : 두 번째 해시 함수로 계산한 탐사 간격(0을 반환해서는 안 됩니다.)
//	i : 탐사 횟수(0, 1, 2, ...)
//	M : 해시 테이블의 크기(버킷 개수)

//장점:2차 군집화 문제 해결 : 각 키마다 H_2(key) 값이 다를 수 있으므로, 동일한 최초 해시 값을 가진 키들이라도 서로 다른 탐사 간격을 가지게 됩니다.
// 따라서, 군집화가 덜 발생하고, 해시 테이블의 탐사 효율이 향상됩니다.

//단점 :두 개의 해시 함수를 설계하고 구현해야 하므로 구현이 더 복잡합니다.
//H_2(key)가 0을 반환하지 않도록 주의해야 합니다.
//H_2(key)와 M이 서로소(공약수가 1뿐인 관계)가 아니면 테이블의 모든 버킷을 탐사하지 못할 수 있습니다.
...
///데이터를 찾을때 밸류보단 키를 통해서 찾는게 일반적
//

//unordered_map<const char*, int> set3;
//
//set3.insert({"gun",2222});
//set3.insert({"ammo", 2500 });
//set3.insert({ "box", 1500 });
//set3.insert({ "shiled", 3000 });
//
//cout << "set3 size : " << set3.size() << endl;
//
//cout << "Load Factor : " << set3.load_factor() << endl;
//
//for (const auto& item : set3)
//{
...
//set4.insert(61);
//
////cout << set4.empty()<< endl;
//
//cout << "set4 size : " << set4.size() << endl;
//for (const auto& num : set4)
//{
//	cout << "Num : " << num << endl;
//}
...
//map<const char*, float> exmap;
//exmap.insert({ "Red", 15 });
//exmap.insert({ "Blue", 16 });
//exmap.insert({ "Green", 18 });
//exmap.insert({ "Pink", 20 });
//
//for (const auto& color : exmap)
//{
//	cout << "Key : " << color.first << "  ,";
//	cout << "Value : " << color.second << endl;
//
//}
//cout << exmap.size() << endl;
